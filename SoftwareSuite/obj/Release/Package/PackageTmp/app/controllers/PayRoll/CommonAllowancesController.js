define(['app'], function (app) {
    app.controller("CommonAllowancesController", function ($scope, $http, $localStorage, $state, AppSettings, SystemUserService, PayRollService) {
        ...
        $scope.ADDAllowance = function () {
            ...
            if (DA_Gazetted > 0) {
                var addAllowance = PayRollService.AddorUpdateAllowance(datatypeid, $scope.FinancialYearID, $scope.MonthID,null, $scope.DepartmentID, DA_Gazetted, HRA, IR, $scope.UserName)
                ...
                var addAllowance = PayRollService.AddorUpdateAllowance(datatypeid, $scope.FinancialYearID, $scope.MonthID, null, $scope.DepartmentID, DA_BoardStaff, HRA, IR, $scope.UserName)
                addAllowance.then(function (response) {
                    try {
                        var res = JSON.parse(response);
                    } catch (err) { }
        ...
        $scope.UpdateAllowance = function (data) {
            ...
            var AddDepartment = PayRollService.AddorUpdateAllowance(datatypeid, data.FinancialYearID, data.MonthID, data.AllowanceID, data.DepartmentID, data.DA, data.HRA, data.IR, $scope.UserName)
