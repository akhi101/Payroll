define(['app'], function (app) {
    app.controller("MobilePensionerPayslipController", function ($scope, $uibModal, $http, $localStorage, $state, AppSettings, SystemUserService, PayRollService) {
        const $ctrl = this;
        $ctrl.$onInit = () => {
        ...
        $scope.GenerateConsolidatedPaySlip = function () {
            ...
            var GetInterimCertificateTobeSignedlocation = PayRollService.GetPensionerConsolidatedPaySlip($scope.FinancialYearID3, 1,1)
            GetInterimCertificateTobeSignedlocation.then(function (response) {
                ///var url = window.location.origin + '/Reports/' + response ;
                console.log(response)
                download(response, 'Pensioner_Consolidated_Payslip.pdf');
